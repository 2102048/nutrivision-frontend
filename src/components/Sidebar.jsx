...
function Sidebar() {
  ...
  const goalsEndRef = useRef(null); // Ref for auto-scroll
  ...
  // Auto-scroll logic when goals are toggled open
  useEffect(() => {
    if (showGoals) {
  ...
  return (
    <aside className="w-72 bg-white border-r border-slate-100 flex flex-col h-screen font-sans sticky top-0">
      {/* Brand Header */}
            ...
            {group.items.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <item.icon className={`w-5 h-5 transition-colors ${isActive ? "text-green-600" : "text-slate-400 group-hover:text-slate-600"}`} />
        ...
        <div className="pt-4 border-t border-slate-50">
          ...
          >
            ...
            <div className="flex items-center gap-2">
              {isManualSource && (
                <span className="text-[9px] bg-white text-slate-500 px-1.5 py-0.5 rounded border border-slate-200 font-bold uppercase">Manual</span>
              )}
              {isBMISource && (
                <span className="text-[9px] bg-indigo-50 text-indigo-600 px-1.5 py-0.5 rounded border border-indigo-100 font-bold uppercase">BMI</span>
              )}
              {showGoals ? <ChevronUpIcon className="w-4 h-4" /> : <ChevronDownIcon className="w-4 h-4" />}
            </div>
          </button>
            ...
            >
              <div className="p-5 bg-white rounded-2xl space-y-6 border border-slate-200 shadow-sm">
                ...
                <div className="space-y-1">
                  <label className="text-[10px] uppercase tracking-wider font-black text-slate-400 block">Daily Calorie Target</label>
                  <div className="flex items-baseline gap-1">
                    <span className="text-3xl font-black text-slate-900 tracking-tight leading-none">{calculatedCalories}</span>
                ...
                <div className="grid grid-cols-3 gap-2.5">
                  {[
                    ...
                    <div key={m.key} className="space-y-2">
                      <label className={`text-[10px] uppercase font-black block leading-none text-center ${m.color}`}>{m.label}</label>
                      <input 
                        type="number" 
                        value={form[m.key]} 
